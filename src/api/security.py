...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if client_ip not in self.request_history:
            self.request_history[client_ip] = {"minute": [], "hour": []}
        ...
        history["minute"] = [t for t in history["minute"] if t > minute_ago]
        history["hour"] = [t for t in history["hour"] if t > hour_ago]

        # 检查限流
        if len(history["minute"]) >= self.requests_per_minute:
            logger.warning(f"IP {client_ip} 触发分钟限流")
            return JSONResponse(status_code=429, content={"error": "Too many requests per minute"})

        if len(history["hour"]) >= self.requests_per_hour:
            logger.warning(f"IP {client_ip} 触发小时限流")
            return JSONResponse(status_code=429, content={"error": "Too many requests per hour"})
