...
class PerformanceMonitor:
    ...
    def record_request(
        self, endpoint: str, duration: float, status_code: int = 200, error: str | None = None
    ) -> None:
        """
        记录请求
...
def timing_decorator(func: Callable) -> Callable:
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            duration = time.perf_counter() - start
            monitor = get_perf_monitor()
            monitor.record_request(func.__name__, duration, error=str(e))
            raise
...
__all__ = ["PerformanceMonitor", "get_perf_monitor", "timing_decorator"]
