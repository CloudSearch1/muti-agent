...
class ResponseCacher:
    ...
    def _generate_key(self, path: str, params: dict | None = None) -> str:
        """
        生成缓存键
    ...
    async def set(self, key: str, data: Any, ttl: int | None = None) -> bool:
        """
        设置缓存
...
# 全局缓存实例
_response_cacher: ResponseCacher | None = None
...
def cache_response(ttl: int = 300):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            # 生成缓存键
            key = cacher._generate_key(func.__name__, kwargs)
