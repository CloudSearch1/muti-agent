...
class NotificationManager:
    ...
    async def send_to_user(self, user_id: int, notification: dict):
        ...
        message = json.dumps(
            {"type": "notification", "data": notification, "timestamp": datetime.now().isoformat()}
        )
    ...
    async def send_task_notification(
        self, user_id: int, task_id: int, task_title: str, status: str
    ):
        """
        发送任务通知
    ...
    async def send_agent_notification(self, user_id: int, agent_name: str, action: str):
        """
        发送 Agent 通知
