...
class SecretManager:
    ...
    def __init__(self, secrets_file: str | None = None):
    ...
    def _load_secrets(self) -> dict:
        ...
        try:
            if self._secrets_file.exists() and self._secrets_file.stat().st_size > 0:
                with open(self._secrets_file, encoding="utf-8") as f:
                    return json.load(f)
    ...
    def _hash_value(self, value: str | None) -> str:
        """哈希值（用于审计）"""
        if not value:
            return "none"
...
def get_secret_manager(secrets_file: str | None = None) -> SecretManager:
    """获取密钥管理器实例"""
